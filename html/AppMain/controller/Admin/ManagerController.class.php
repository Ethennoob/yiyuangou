<?php
    ...
    public function roleList(){
        $pageInfo=$this->P();
        $class=$this->table('sys_admin_role')->order('id desc');
    ...
    public function getRoleDetail(){
        ...
        $role=$this->table('sys_admin_role')->where(['id'=>$roleID])->get(['id','role_name','add_time','update_time'],true);
    ...
    public function editRole(){
        ...
        $roleID=intval($_POST['role_id']);
        $role=$this->table('sys_admin_role')->where(['id'=>$roleID])->get(['id'],true);
        if (!$role){
            $this->R('','70005');
        }
        
        $role=$this->table('sys_admin_role')->where(['id'=>$roleID])->update(['role_name'=>$_POST['role_name'],'update_time'=>time()]);
        if (!$role){
            $this->R('','40001');
        }
    ...
    public function addRole(){
        $this->V(['role_name'=>[]]);
        $role=$this->table('sys_admin_role')->save(['role_name'=>$_POST['role_name'],'add_time'=>time()]);
    ...
    public function getRoleAuth(){
        ...
        $myAuth=$this->table('sys_admin_role')->where(['id'=>$roleID])->get(['auth'],true);
    ...
    public function setRoleAuth(){
        ...
        $myAuth=$this->table('sys_admin_role')->where(['id'=>$roleID])->update(['auth'=>$data]);
    ...
    public function authList($type='json'){
        $authList=$this->table('sys_auth')->order('pid ASC, id ASC')->get(null);
    ...
    public function addAuth(){
        ...
        if ($pid > 0){
            $isMold=$this->table('sys_auth')->where(['id'=>$pid])->get(null,true);
            if (!$isMold){
                $this->R('','70009');
            }
        }
        ...
        $add=$this->table('sys_auth')->save($data);
        if (!$add){
            $this->R('','40001');
        }
    ...
    public function editAuth(){
        ...
        $update=$this->table('sys_auth')->where(['id'=>intval($_POST['id'])])->update($data);
        if (!$update){
            $this->R('','40001');
        }
    ...
    public function deleteAuth(){
        ...
        $update=$this->table('sys_auth')->where('id='.$moldID.' or pid='.$moldID)->delete();
        if (!$update){
            $this->R('','40001');
        }
