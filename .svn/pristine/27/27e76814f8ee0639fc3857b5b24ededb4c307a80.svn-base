...
class Gd{
    ...
    public function open($imgname){
        ...
        if('gif' == $this->info['type']){
            $this->gif = new \System\lib\Image\GIF($imgname);
    ...
    public function water($source, $locate = Image::IMAGE_WATER_SOUTHEAST,$alpha=80){
        ...
        if(empty($this->img)) {
            $this->error=true;
            $this->errorInfo='没有可以被添加水印的图像资源';
            return false;
        }
        if(!is_file($source)) {
            $this->error=true;
            $this->errorInfo='水印图像不存在';
            return false;
        }
    ...
    public function text($text, $font, $size, $color = '#00000000', 
        ...
        if(empty($this->img)) {
            $this->error=true;
            $this->errorInfo='没有可以被写入文字的图像资源';
            return false;
        }
        if(!is_file($font)) {
            $this->error=true;
            $this->errorInfo="不存在的字体文件：{$font}";
            return false;
        }
