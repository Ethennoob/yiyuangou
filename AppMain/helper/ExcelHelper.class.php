...
class ExcelHelper extends BaseHelper {
	...
	function PHPExcelGood($list,$data){
		$this->vendor('Excel.PHPExcel');
		//创建PHPExcel对象
		$objPHPExcel = new \PHPExcel();
		//设置属性
		...
		$objPHPExcel->setActiveSheetIndex(0)
		->setCellValue('A1', $data['title'])
		->setCellValue('A2', $data['nickname'])
		->setCellValue('B2', $data['phone'])
		->setCellValue('C2', $data['num'])
		->setCellValue('D2', $data['add_time'])
		->setCellValue('E2', $data['code'])
		->setCellValue('F2', $data['remask']);
		// 内容
		for ($i = 0, $len = count($list); $i < $len; $i++) {
			//转换成字符串
			$objPHPExcel->getActiveSheet(0)->setCellValueExplicit('A' . ($i + 3), $list[$i]['nickname'],\PHPExcel_Cell_DataType::TYPE_STRING);
			$objPHPExcel->getActiveSheet(0)->setCellValue('B' . ($i + 3), $list[$i]['phone']);
			$objPHPExcel->getActiveSheet(0)->setCellValue('C' . ($i + 3), $list[$i]['num']);
			$objPHPExcel->getActiveSheet(0)->setCellValue('D' . ($i + 3), $list[$i]['add_time']);
			$objPHPExcel->getActiveSheet(0)->setCellValue('E' . ($i + 3), $list[$i]['code']);
			$objPHPExcel->getActiveSheet(0)->setCellValue('F' . ($i + 3), $list[$i]['remask']);
