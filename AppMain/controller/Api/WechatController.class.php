...
class WechatController extends BaseClass {
    ...
    private function isUser() {
        ...
        //用户是否存在
        //$isUser = $this->table('user')->where(['openid_line' => $openid])->get(['id', 'parent', 'parent_share_type', 'activity_id'], true);
        $isUser = $this->table('user')->where(['openid_line' => $openid])->get(['id'], true);
        if (!$isUser) {
            return false;
        }
        return $isUser;
