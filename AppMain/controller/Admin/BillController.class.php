<?php 
	...
	class BillController extends BaseClass {
	    ...
	    $field = ['id','goods_id','user_id','thematic_id','is_confirm','status','bill_sn','gameID','game_img','add_time','update_time'];
	        //查询并分页
		...
		public function billOneDetail(){
            ...
            if(!$bill){
                $this->R('',70009);
	                ...
	                $status = $this->table('goods')->where(['is_on'=>1,'id'=>$bill['goods_id']])->get(['goods_name','goods_thumb','cost_price','price','goods_sn','nature'],true);
	                ...
	                $status = $this->table('thematic')->where(['is_on'=>1,'id'=>$bill['thematic_id']])->get(['thematic_name'],true);
	    ...
	    public function phone(){
	     ...
	     public function addGamesID(){
            ...
            if ($goods['nature']==0||$goods['nature']==1) {
            	$this->R('',40001);
            }
            $bill = $this->table('bill')->where(['id'=>$data['bill_id'],'is_on'=>1])->update(['gameID'=>$data['gameID']]);
            if(!$bill){
                    $this->R('',40001);
                }
	    ...
	    public function addGamesImg(){
	    	$rule = ['bill_id' =>['egNum',null,true]];
            $this->V($rule); 
            foreach ($rule as $k => $v) {
                        $data[$k] = $_POST[$k];
                }
	    	//图片上传
            $pictureName = $_FILES['game_img'];
            $imgarray = $this->H('PictureUpload')->pictureUpload($pictureName,'game',false);
            $data['game_img'] = $imgarray['img'];
            $value = $this->table('bill')->where(['id'=>$data['bill_id'],'is_on'=>1])->update(['game_img'=>$data['game_img']]);
            if (!$value) {
                    $this->R('',40001);
            }
            $this->R();
	    }
	    public function editGamesImg(){
	    	$rule = ['bill_id' =>['egNum',null,true]];
            $this->V($rule);
            foreach ($rule as $k => $v) {
                        $data[$k] = $_POST[$k];
            ...
            $game_img = $this->table('bill')->where(['id'=>$data['bill_id'],'is_on'=>1])->get(['game_img'],true);
            //分割图片url，匹配图片名是否相同，同则不删除，不同则删除
                @unlink("../html".$game_img['game_img']);
                $pictureName=$_FILES['game_img'];
                $imgarray=$this->H('PictureUpload')->pictureUpload($pictureName,'game',false);
                $data['game_img']=$imgarray['img'];
           }
		...
		public function gameOneDetail(){
            ...
            $bill = $this->table('bill')->where(['is_on'=>1,'id'=>$id])->get(['gameID','game_img','user_id'],true);
